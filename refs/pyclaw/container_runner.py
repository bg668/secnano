...
def _forward_container_env_args() -> list[str]:
    args: list[str] = []
    forwarded = {
        "NAOCLAW_PY_MOCK_AGENT": "1" if MOCK_AGENT_ENABLED else None,
        "ANTHROPIC_MODEL": ANTHROPIC_MODEL or None,
        "AGENT_MAX_TOKENS": AGENT_MAX_TOKENS or None,
        "AGENT_MAX_TURNS": AGENT_MAX_TURNS or None,
    }
...
def _forward_runner_env() -> dict[str, str]:
    env = os.environ.copy()
    env["TZ"] = getattr(TIMEZONE, "key", str(TIMEZONE))
    if MOCK_AGENT_ENABLED:
        env["NAOCLAW_PY_MOCK_AGENT"] = "1"
    if ANTHROPIC_MODEL:
        env["ANTHROPIC_MODEL"] = ANTHROPIC_MODEL
    if AGENT_MAX_TOKENS:
        env["AGENT_MAX_TOKENS"] = AGENT_MAX_TOKENS
    if AGENT_MAX_TURNS:
        env["AGENT_MAX_TURNS"] = AGENT_MAX_TURNS
    return env


def _ensure_path_binding(source: Path, target: Path) -> None:
...
def _prepare_local_workspace(group: RegisteredGroup, mounts: list[VolumeMount]) -> Path:
    workspace_base = DATA_DIR / "local-runner" / group.folder / "workspace"
    workspace_base.mkdir(parents=True, exist_ok=True)

    for mount in mounts:
        if not mount.containerPath.startswith("/workspace/"):
            continue
        relative_path = mount.containerPath.removeprefix("/workspace/")
        if not relative_path:
            continue
        _ensure_path_binding(Path(mount.hostPath), workspace_base / relative_path)
...
def _build_volume_mounts(group: RegisteredGroup, is_main: bool) -> list[VolumeMount]:
    ...
    # Sync skills
    skills_src = CONTAINER_DIR / "skills"
    ...
    agent_runner_src = CONTAINER_DIR / "agent-runner" / "src"
...
def _build_container_args(
    ...
    args += ["-e", f"TZ={tz_name}"]
    args += _forward_container_env_args()
...
async def run_container_agent(
    ...
    safe_prefix = re.sub(r"[^a-zA-Z0-9-]", "-", CONTAINER_NAME_PREFIX)
    runner_name = f"{safe_prefix}-{safe_name}-{int(time.time() * 1000)}"

    proc_env = None
    proc_cwd = None
    if LOCAL_AGENT_RUNNER:
        workspace_base = _prepare_local_workspace(group, mounts)
        container_args = [
            sys.executable,
            str(CONTAINER_DIR / "agent-runner" / "agent_runner.py"),
        ]
        proc_env = _forward_runner_env()
        proc_env["NAOCLAW_WORKSPACE_BASE"] = str(workspace_base)
        proc_cwd = str(PROJECT_ROOT)
    else:
        container_args = _build_container_args(mounts, runner_name)

    logger.info(
        f"Spawning agent runner group={group.name} runner={runner_name} "
        f"mounts={len(mounts)} isMain={inp.isMain} localRunner={LOCAL_AGENT_RUNNER}"
    )
    ...
    on_process(proc, runner_name)  # type: ignore[arg-type]
    ...
    try:
        ...
        timed_out = True
        logger.error(f"Container timeout group={group.name} runner={runner_name}")
        try:
            if LOCAL_AGENT_RUNNER:
                proc.kill()
            else:
                subprocess.run(
                    [CONTAINER_RUNTIME_BIN, "stop", runner_name],
                    timeout=15,
                    capture_output=True,
                )
        except Exception:
