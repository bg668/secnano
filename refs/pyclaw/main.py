...
from .bootstrap import bootstrap_project_layout, build_default_local_group
from .channels import cli as _cli_channel  # noqa: F401
from .config import (
    ASSISTANT_NAME,
    IDLE_TIMEOUT,
    LOCAL_AGENT_RUNNER,
...
async def _ensure_default_local_group() -> None:
    default_group = build_default_local_group(_registered_groups)
    if default_group is None:
        return

    jid, group = default_group
...
async def _process_group_messages(chat_jid: str) -> bool:
    ...
    async def _on_output(result: ContainerOutput) -> None:
        ...
        if result.status == "error" and result.error:
            await channel.send_message(chat_jid, f"[agent error] {result.error}")
            output_sent = True
...
async def _run_agent(
    ...
    try:
        ...
        if on_output:
            await on_output(
                ContainerOutput(status="error", result=None, error=str(exc))
            )
        return "error"
...
async def main() -> None:
    ...
    logger.info(f"Starting extracted NanoClaw Python runtime root={PROJECT_ROOT}")
    if LOCAL_AGENT_RUNNER:
        logger.info("LOCAL_AGENT_RUNNER enabled; container runtime check skipped")
    else:
        try:
            ...
            logger.warning(
                "Container runtime unavailable at startup; host runtime will still boot, "
                f"but agent runs will fail until it is available: {exc}"
            )

    await init_database()
    logger.info("Database initialized")
    await _load_state()
    await _ensure_default_local_group()
    ...
    # Builtin channel plugins self-register on import. The local CLI plugin is
    # imported at module load time to guarantee at least one debug-friendly
    # channel in the extracted learning repo.
    ...
    if not _channels:
        logger.critical(
            "No channels connected. Enable LOCAL_CHANNEL_ENABLED or add another channel plugin."
        )
        sys.exit(1)
